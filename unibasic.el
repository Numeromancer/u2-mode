...
It is definately preferable to have this set if possible."
    :type '(choice (const null) boolean)
    :group 'unibasic )
  (defcustom unibasic-header ""
    "Value for Auto-Insert for UniBasic files.
...
(defvar unibasic-compiler-directive-regexp
  "^\\s-*\\$"
  "Regular expression for recognizing a compiler directive, such as an include
or equate.  These are pushed to the left.")
...
"
  ...
  (setq indent-line-function 'unibasic-indent-line ;; for indent-region
        ...
        ;; Do not override user-settable option
...
(defun unibasic-electric-dollar (&optional count)
  "Insert '$' and indent to column 0 if this is an empty line."
  (interactive "p")
  (insert-char last-command-char count)
  (if (save-excursion (beginning-of-line) (looking-at "^[ \t]*\\$"))
      (save-excursion (beginning-of-line) (delete-horizontal-space))))
...
to `unibasic-initial-indent' plus `unibasic-default-indent' as needed."
  ...
  (let* ((line (unibasic-syntactic-line 0)) ; find the last syntactic line
    ...
    (if (or (save-excursion;; look for block starts, skipping comments.
              (beginning-of-line (1+ line))
              (re-search-backward unibasic-block-start-regexp
                              (unibasic-get-bol 0) t))
            (save-excursion;; look for block starts, skipping comments.
              (beginning-of-line (1+ line))
              (re-search-backward unibasic-block-inter-regexp
                                  (unibasic-get-bol 0) t)))
    ...
    (if (or (unibasic-looking-at unibasic-block-end-regexp)
            (unibasic-looking-at unibasic-block-inter-regexp))
        (setq thiscol (- thiscol unibasic-default-indent)))
    (cond
     ...
     (t
      (unibasic-get-arg-align thiscol)))))
    ...
    (save-excursion
      ...
      (while (looking-at equate-regexp)
        (let* ((thisn (- (match-end 1) (match-beginning 1)))
          ...
          (replace-match (concat "EQUATE \\1" spaces " TO \\2 \\3"))
          (unibasic-indent-line)
          (forward-line))))))
          
      

(defun unibasic-align-command-pairs ()
  (interactive)
  (let ((commands-regexp "[ \t]*\\([^;\n]+=[^;\n]+;\\)[ \t]*\\([^;\n]+=[^;\n]+\\)[ \t]*\\(;[ \t]*[*!].*\\)?$")
    ...
    (save-excursion
      ...
      (while (and (< (line-end-position) (point-max)) (looking-at commands-regexp))
        (let ((thisn1 (- (match-end 1) (match-beginning 1)))
          ...
          (and (> thisn2 longest-second) (setq longest-second thisn2))
          (forward-line)))
      (setq eoe (point))
      (goto-char boe)
      (while (and (> (line-end-position) (point-min)) (looking-at commands-regexp))
        (let* ((thisn1 (- (match-end 1) (match-beginning 1)))
               ...
               (spaces2 (make-string (- longest-second thisn2) ? )))
          (replace-match (concat "\\1" spaces1 " \\2" spaces2 "\\3"))
          (unibasic-indent-line)
          (forward-line))))))
...
(eval-and-compile
  (defvar unibasic-special-words
    '( "@AM" "@VM" "@SM" "@FM" "@RM" "@TM" "@SVM" "@ACCOUNT" "@COMMAND"
       "@CONV" "@CRTHIGH" "@CRTWIDE" "@DATA" "@DATE" "@NULL"
