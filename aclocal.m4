dnl Almost exclusively stolen from Tramp.
...
(if (or (< emacs-major-version 21)\
    ...
    \"ok\")\
"
  AC_SUBST(U2_MODE_EMACS_VERSION_CHECK)

  AC_MSG_CHECKING([for $EMACS version])
  AC_EMACS_LISP(emacs_version, $U2_MODE_EMACS_VERSION_CHECK, "noecho")
  ...
  if test "${EMACS_cv_SYS_$1}" = "nil"; then
    if test "${USE_CONTRIB}" = "yes"; then
      if test -e contrib/$library; then
        U2_MODE_CONTRIB_FILES="$library $U2_MODE_CONTRIB_FILES"
...
dnl	AC_CONFIG_LINKS(lisp/$library:contrib/$library)
      ...
      AC_MSG_RESULT(not found)
      AC_MSG_ERROR(Use --with-contrib for implementation supplied with U2-mode)
    else
...
dnl
dnl Checks whether u2-mode is prepared for (X)Emacs package.  This case,
...
dnl       [[DATADIR/xemacs/site-lisp]]]))
  ...
  dnl Expand $lispdir_default for u2-modeinst.texi.  We need to apply it
...
AC_DEFUN(AC_PATH_INFODIR, [
  ...
  dnl Expand $infodir_default for u2-modeinst.texi.  We need to apply it
  dnl several times, because $prefix, $datarootdir and $datadir need
  dnl to be expanded in an unknown order.
